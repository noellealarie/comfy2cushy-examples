...
action({
    ...
    ui: (form) => {
        ...
        return {
            ...
            hiresfix: form.group({
                label: 'Hires Fix',
                items: () => ({
                    scale_factor: form.float({ label: 'Scale Factor', default: 1, theme: 'input' }),
                    ...x.kSampler(),
                    seed: form.seed({ label: 'Seed', defaultMode: 'randomize' }),
                }),
            }),
    ...
    run: async (flow, p) => {
        ...
        if (hiresfix) {
            ...
            const final_w = width * hiresfix.scale_factor
            const final_h = height * hiresfix.scale_factor
            const UPSCALED_LATENT = graph.LatentUpscale({
                samples: LATENT,
                crop: 'disabled',
